...
export class MemoFetcher {
	...
	static getFormattedMemo(memo: MemoEntry): string {
	...
	private async formatMemos(doc: TextDocument): Promise<void> {
		...
		for (const memo of memos)
			edit.replace(doc.uri, [memo.offset, memo.offset + memo.rawLength], MemoFetcher.getFormattedMemo(memo));
		await edit.apply({ isRefactoring: true });
	}

	private handleTabChange(changed: readonly TabGroup[]): void {
